...
```cmd
>Debian.exe run uname -r
4.4.0-43-Microsoft
```

#### Run with command line with path translation
```cmd
>Debian.exe runp echo C:\Windows\System32\cmd.exe
/mnt/c/Windows/System32/cmd.exe
```

#### Change Default User(id command required)
```cmd
>Debian.exe config --default-user user
...
```cmd
>Debian.exe config --default-term wt
```
...
```cmd
>Debian.exe backup backup.tar.gz
```
export to backup.ext4.vhdx.gz  (WSL2 only)
```cmd
>Debian.exe backup backup.ext4.vhdx.gz
```
